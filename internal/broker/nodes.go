...
// it the Broker publishes no reusable candidate, which is what a deployment with
// no node agents actually has: the machines a capacity connection lists are not
// candidates until an agent enrolls on one.
