...
func TestCreateRunReturnsInternalErrorWhenInitialPersistenceFails(t *testing.T) {
	...
	handler := New(Deps{
		Orchestrator: orchestrator.New(log, scheduler.New(), provider, activeTestWorkspaces),
		Offers:       singleProviderOffers{provider: provider},
		Workloads:    workload.New(log, activeTestWorkspaces),
	})
...
func TestCreateRunEnvOverridesStoredWorkloadRevision(t *testing.T) {
	...
	handler := New(Deps{Orchestrator: orch, Offers: singleProviderOffers{provider: ad}, Workloads: workload.New(log, activeTestWorkspaces), Resolver: ociresolver.NewStaticResolver(nil)})
...
func newHTTPTestServerWithOptions(t *testing.T, options ...Option) http.Handler {
	...
	return New(Deps{Orchestrator: orch, Offers: singleProviderOffers{provider: ad}, Workloads: workload.New(log, activeTestWorkspaces), Resolver: resolver}, options...)
}
