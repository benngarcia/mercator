...
func TestSinkReplayAPIAndFailureIsolation(t *testing.T) {
	...
	handler := New(Deps{Orchestrator: orch, Offers: singleProviderOffers{provider: ad}, Workloads: workload.New(workspaceTestLog{EventLog: log}), Sinks: sinks.NewManager(log, map[string]sinks.Sink{"audit": failingSink{}}), Resolver: ociresolver.NewStaticResolver(nil)})
