...
func (a *Adapter) Observe(ctx context.Context, req adapter.ObserveRequest) (adapter.ExternalObservation, error) {
	...
	// No start moment, for the reason Shadeform states about created_at and RunPod
	// about lastStartedAt: start_date is when Vast started this instance's contract,
	// which is before the image has landed and before anything runs on it, and it
	// does not move when the container process begins. An instance record is a fact
	// about a machine, and when a container got a process is a fact about a
	// container runtime. Reporting the contract's start as the workload's would fold
	// a whole image pull into the runtime and calibrate the start estimator on close
	// to nothing.
