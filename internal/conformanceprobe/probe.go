...
func Run(ctx context.Context, args []string, env map[string]string, _ io.Writer, stderr io.Writer) int {
	scenario, exitCode, err := parseScenario(args)
	if err != nil {
	...
	if err := reporter.post(ctx, report{Type: "ready", Data: map[string]any{
		"scenario": scenario,
		"goos":     runtime.GOOS,
		"goarch":   runtime.GOARCH,
	}}); err != nil {
		if scenario == "wait-for-cancel" && ctx.Err() != nil {
			return 0
		}
		_, _ = fmt.Fprintln(stderr, err)
		return 1
...
func parseScenario(args []string) (string, int, error) {
	if len(args) == 1 && (args[0] == "success" || args[0] == "wait-for-cancel") {
		return args[0], 0, nil
	}
	if len(args) == 3 && args[0] == "failure" && args[1] == "--exit-code" {
		code, err := strconv.Atoi(args[2])
		if err != nil || code <= 0 || code > 255 {
			return "", 0, errors.New("failure exit code must be between 1 and 255")
		}
		return "failure", code, nil
	}
	return "", 0, errors.New("usage: mercator-conformance-probe success | failure --exit-code CODE | wait-for-cancel")
}
