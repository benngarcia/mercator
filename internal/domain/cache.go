...
// where it is and who may see it. Three rules separate it from an Artifact:
//
//   - its identity is its workspace-scoped name. Two workspaces that both call a
//     cache "compiler-cache" name two different caches, and nothing may ever
//     hand one of them the other's bytes. That isolation is hard rather than
//     best-effort, because a mutable cache is exactly where one tenant's build
//     output would leak into another tenant's build;
...
// choose which workspace it runs in: the Run's own workspace scopes every cache
// it names.
...
// three parts of the identity rather than from the name alone, which is what
// makes cross-workspace sharing impossible by construction instead of by a
// comparison somebody has to remember to make.
func CacheIdentity(workspaceID string, requirement CacheMountRequirement) string {
	return workspaceID + "/" + requirement.Name + "/" + requirement.CompatibilityKey
}

// CacheVolumeName is what a container runtime calls the durable volume holding
// one cache. The workspace and the name are spelled out so an operator reading
// `docker volume ls` can see whose cache this is, and the digest of the full
// identity is appended because a compatibility key is an application's own
// string that no volume name can carry literally.
func CacheVolumeName(workspaceID string, requirement CacheMountRequirement) string {
	sum := sha256.Sum256([]byte(CacheIdentity(workspaceID, requirement)))
	return "mercator-cache-" + workspaceID + "-" + requirement.Name + "-" + hex.EncodeToString(sum[:4])
}
...
type CacheMount struct {
	// WorkspaceID is who owns this cache. It is part of the identity rather
	// than a label on it.
...
func (mount CacheMount) Identity() string {
	return CacheIdentity(mount.WorkspaceID, CacheMountRequirement{
		Name:             mount.Name,
...
// Held answers which generation is under one workspace-scoped name now. A host
...
// done this work.
func (inventory CacheInventory) Held(workspaceID, name string) (CacheMount, bool) {
	var newest CacheMount
	found := false
	for _, mount := range inventory.Mounts {
		if mount.WorkspaceID != workspaceID || mount.Name != name {
			continue
		}
...
// Holds reports whether this host has a cache a Run may reuse: the same
// workspace, the same name, and the generation the application asked for. It is
...
func (inventory CacheInventory) Holds(workspaceID string, requirement CacheMountRequirement) bool {
	for _, mount := range inventory.Mounts {
		if mount.WorkspaceID == workspaceID &&
			mount.Name == requirement.Name &&
...
func CacheWarmth(workspaceID string, required []CacheMountRequirement, inventory CacheInventory) []CacheEvidence {
	if len(required) == 0 {
		return nil
	}
	...
	for _, requirement := range required {
		...
		switch held, isHeld := inventory.Held(workspaceID, requirement.Name); {
		case !inventory.Known:
			found.Locality = LocalityUnknown
		case inventory.Holds(workspaceID, requirement):
