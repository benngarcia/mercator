...
func TestLocalLoginPreservesDeepLink(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/auth/login?next=%2Fcanvas%3Frun_id%3Drun_local", nil)
	recorder := httptest.NewRecorder()
	...
	if location := recorder.Header().Get("Location"); location != "/canvas?run_id=run_local" {
