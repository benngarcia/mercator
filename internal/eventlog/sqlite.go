...
func refuseFlattenedStreamCollisions(ctx context.Context, tx *sql.Tx) error {
	...
	if err == nil {
		return fmt.Errorf("eventlog: cannot remove workspace partition from duplicate stream %s/%s at version %d", streamType, streamID, streamVersion)
	}
...
func refuseFlattenedCommandCollisions(ctx context.Context, tx *sql.Tx, eventsPartitioned bool) error {
	query := `SELECT command_appends.command_key
		...
		WHERE EXISTS (
			...
			WHERE events.command_key = command_appends.command_key
			  AND events.global_position BETWEEN command_appends.first_position AND command_appends.last_position
		)
	...
	if err == nil {
		return fmt.Errorf("eventlog: cannot remove workspace partition from duplicate command key %q", commandKey)
	}
	if !errors.Is(err, sql.ErrNoRows) {
		return err
	}
...
func migratePartitionedEvents(ctx context.Context, tx *sql.Tx) error {
	for _, statement := range []string{
		...
		if _, err := tx.ExecContext(ctx, statement); err != nil {
			return fmt.Errorf("eventlog: remove workspace partition from events: %w", err)
		}
	}
	return nil
}
	...
	for _, statement := range []string{
		...
		WHERE EXISTS (
			...
			WHERE events.command_key = command_appends_workspace_legacy.command_key
			  AND events.global_position BETWEEN command_appends_workspace_legacy.first_position AND command_appends_workspace_legacy.last_position
		)`,
		`DROP TABLE command_appends_workspace_legacy`,
	} {
		if _, err := tx.ExecContext(ctx, statement); err != nil {
			return fmt.Errorf("eventlog: remove workspace partition from commands: %w", err)
		}
	}
	return nil
}
