...
func New(ctx context.Context, db *sql.DB) (*Storage, error) {
	if err := flattenWorkspaceStorage(ctx, db); err != nil {
		_ = db.Close()
		return nil, err
	}
	...
	storage := &Storage{db: db, log: sqliteLog, credentials: credentials}
	storage.schedules = &RentalScheduleStore{db: db, log: sqliteLog}
