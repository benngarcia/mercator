...
func TestRunListUsesTheDeployment(t *testing.T) {
...
func TestLatestRunTraversesEveryProjectionPage(t *testing.T) {
	...
	runID, err := (&session{baseURL: server.URL, client: server.Client()}).latestRun(context.Background())
...
// Asking for one run when the deployment has none must say so plainly instead
...
func TestRunGetSaysWhenThereIsNoRunToDefaultTo(t *testing.T) {
	...
	if code == 0 {
		t.Fatal("expected a failure when the deployment has no runs")
	}
