...
// should have to name only what is genuinely ambiguous, so the run resolves
// on demand whenever there is exactly one sensible answer.
type session struct {
	baseURL string
	token   string
	client  *http.Client
...
// latestRun returns the most recent run, which is what "the run
// I just started" means at a prompt. Run ids are UUIDv7, so the id order the
// API returns is creation order.
func (s *session) latestRun(ctx context.Context) (string, error) {
	var page struct {
		Runs []struct {
			ID string `json:"id"`
		} `json:"runs"`
	}
	if err := s.get(ctx, "/v1/runs", nil, &page); err != nil {
		return "", fmt.Errorf("look up runs: %w", err)
	}
	if len(page.Runs) == 0 {
		return "", fmt.Errorf("this deployment has no runs yet")
	}
	return page.Runs[len(page.Runs)-1].ID, nil
}

// soleConnection returns the deployment's only connection. A deployment with one
...
// by retrying.
func (s *session) soleConnection(ctx context.Context) (string, error) {
	var page struct {
		Connections []struct {
			ID string `json:"id"`
		} `json:"connections"`
	}
	if err := s.get(ctx, "/v1/connections", nil, &page); err != nil {
		return "", fmt.Errorf("look up connections: %w", err)
	}
	switch len(page.Connections) {
	case 0:
		return "", fmt.Errorf("this deployment has no connections")
	case 1:
	...
	return "", fmt.Errorf("this deployment has %d connections; pass --connection-id (%s)",
		len(page.Connections), strings.Join(names, ", "))
}
