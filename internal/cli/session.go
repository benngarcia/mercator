...
// it authenticates, and the facts it can discover for itself.
type session struct {
	baseURL string
	token   string
	client  *http.Client
...
func (s *session) soleConnection(ctx context.Context) (string, error) {
	...
	return "", fmt.Errorf("this deployment has %d connections; pass --connection-id (%s)", len(page.Connections), strings.Join(names, ", "))
}
