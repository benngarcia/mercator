...
// starve the rest of the deployment: a run that always fails to advance is
...
func TestAdvanceOpenRunsContinuesPastFailingRun(t *testing.T) {
	...
	// on every attempt.
	seedWedgedRun(t, orch.log, "run_wedged")
	createRun(t, ctx, orch)
...
func seedWedgedRun(t *testing.T, log eventlog.EventLog, runID string) {
	t.Helper()
		...
		Events: []eventlog.NewEvent{
			{
				...
				// to clean up. A request naming no Run wedges the deployment's
