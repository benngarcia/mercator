...
func TestPreviewPlacementSharesOfferQueryPathWithDecide(t *testing.T) {
	...
	orch := New(workspaceTestLog{EventLog: log}, scheduler.New(), ad, withTestCapacity())
...
func newIntakeOrchWithAdapter(t *testing.T, provider Adapter) *Orchestrator {
	...
	return New(workspaceTestLog{EventLog: log}, scheduler.New(), provider, withTestCapacity())
}
