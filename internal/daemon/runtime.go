...
type Config struct {
	...
	// ProviderFactory replaces the production provider registry in focused
	// lifecycle tests. Production callers leave it nil.
...
func New(ctx context.Context, cfg Config) (_ *Runtime, err error) {
	...
	if factory == nil {
		factory = providers.Default().Factory()
	}
...
func (r *Runtime) Shutdown(ctx context.Context) error {
	r.shutdownOnce.Do(func() {
		...
		case <-ctx.Done():
			reconcileErr = fmt.Errorf("daemon: join reconciliation: %w", ctx.Err())
		}
		storageErr := r.storage.Close()
		r.shutdownErr = errors.Join(httpErr, reconcileErr, storageErr)
	})
...
// ReconcileWorkspace drives open Runs and the orphan janitor once, then returns
// the provider inventory observed after both cleanup paths have run.
