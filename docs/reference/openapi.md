...
`GET /v1/runs` reads the indexed Run projection in stable Run-id order.
`limit` accepts 1 through 100 and defaults to 50. When `next_cursor` is
present, pass it back unchanged as `cursor` to read the next page.
