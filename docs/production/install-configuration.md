...
export MERCATOR_TRANSPORT_BOUNDARY=tls-proxy
export MERCATOR_PUBLIC_URL=https://mercator.example.com
export MERCATOR_ADMIN_ADDR=127.0.0.1:8081
```

`tls-proxy` requires an absolute `https://` public URL and cannot be combined
with process TLS material. A broker used only inside an un-published private
container network instead declares `MERCATOR_TRANSPORT_BOUNDARY=private-network`
and must not set `MERCATOR_PUBLIC_URL`. Any other value refuses startup.
...
| `MERCATOR_TRANSPORT_BOUNDARY` | none | Set to `tls-proxy` when a TLS proxy reaches Mercator through a private container network, or `private-network` for an internal-only container service. The former requires an HTTPS `MERCATOR_PUBLIC_URL`; the latter forbids one. Never publish the plaintext service port. |
